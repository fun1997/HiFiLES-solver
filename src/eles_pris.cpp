...
void eles_pris::setup_ele_type_specific()
{
  ...
  if(run_input.shock_cap)
  {
    if (run_input.shock_det == 0) //persson
      ...
      FatalError("Shock detector not implmented.");
    if (run_input.shock_cap == 1) //exp filter
      set_exp_filter();
    else
      FatalError("Shock capturing method not implmented.");
  }
    ...
    for (l = 0; l < 2 * order + 1; l++) //sum of x,y,z mode
    {
      for (k = 0; k < l + 1; k++) //k<=sum
      {
        for (j = 0; j < l - k + 1; j++) //j<=sum-k
        {
          ...
          {
            eta_rs = (double)(i + j) / (double)(order);
            eta_t = (double)(k) / (double)(order);
            exp_filter(mode, mode) = 1.;
            if (eta_rs > eta_c)
              exp_filter(mode, mode) *= exp(-run_input.expf_fac * pow((eta_rs - eta_c) / (1. - eta_c), run_input.expf_order));
            if (eta_t > eta_c)
              exp_filter(mode, mode) *= exp(-run_input.expf_fac * pow((eta_t - eta_c) / (1. - eta_c), run_input.expf_order));
            mode++;
          }
        }
      }
