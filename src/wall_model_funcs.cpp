...
void calc_wall_stress(hf_array<double> &in_u_wm, hf_array<double> &in_uw, double in_dist, hf_array<double> &in_norm, hf_array<double> &out_fn)
{
    ...
    hf_array<double> vw(n_dims), v_wm(n_dims), v_wm_rel(n_dims), tw(n_dims);
    ...
    for (int i = 0; i < n_dims; i++)
        v_wm_n += in_u_wm(i + 1) / rho_wm * in_norm(i); //wall normal input velocity

    v_wm_mag = 0;
    v_wm_rel_mag = 0;
    for (int i = 0; i < n_dims; i++)
    {
        v_wm(i) = in_u_wm(i + 1) / rho_wm - in_norm(i) * v_wm_n; //wall parallel input velocity
        vw(i) = in_uw(i + 1) / rho_w;                            // wall velocity
        v_wm_rel(i) = v_wm(i) - vw(i);                           //relative velocity
        v_wm_rel_mag += v_wm_rel(i) * v_wm_rel(i);               //|u_wm-u_w|
        v_wm_mag += v_wm(i) * v_wm(i);                           //|u_wm|
    }
    ...
    for (int i = 0; i < n_dims; i++)
    {
        ke_wm += 0.5 * pow(in_u_wm(i + 1) / rho_wm, 2);
        ke_w += 0.5 * pow(vw(i), 2);
    }
    ...
    if (run_input.wall_model == 1) //Werner-Wengle
    {
        ...
        for (int i = 0; i < n_dims; i++)
            tw(i) = tw_mag * v_wm_rel(i) / v_wm_rel_mag; //stress vector parallel to wall

        // Wall heat flux
        if (Rey < Rey_c)
            qw = (inte_w - inte_wm) * run_input.gamma * tw_mag / (run_input.prandtl * v_wm_rel_mag);
        else
            qw = (inte_w - inte_wm) * run_input.gamma * tw_mag / (run_input.prandtl_t * (v_wm_rel_mag + utau * 11.81 * (run_input.prandtl / run_input.prandtl_t - 1.0)));
    }
